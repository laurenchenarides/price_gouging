...
# ------------------------------------------------------------------------------
# Audit
# C1. Uniform-pricing pairwise join drops mixed-enforcement store pairs (the paper's identifying contrast)
# - **File:** `code/06_uniform_pricing.R`, `make_pairwise_logdiff()`, ~lines 60–80.
# - **Issue:** The self-join that builds store pairs uses join keys `c("retailer", "product", "week_seq", "week_start", "period", "month_fe")`. `period` (Pre/During/Post-SOE) is *state-specific*: after Louisiana's SOE expired (Mar 2021) while Mississippi's was active (until Nov 2021), an LA store is "Post-SOE" and an MS store is "During SOE" in the same calendar week. Because `period` is a join key, **within-chain pairs that straddle enforcement status are silently excluded** from `Diff_bar`. The paper (§5.1) states that exactly these pairs — "a retailer operating stores in both an active-enforcement state and a post-enforcement state during the same week" — identify β₂.
# - **Second-order effect:** In weeks where a chain has stores in both statuses, the collapse `group_by(retailer, product, week_seq, period)` creates **two rows per retailer-product-week** (one "During", one "Post"), each entering the regression as a separate observation with its own during/post dummy. This plausibly explains why N = 4,467 exceeds the ~4,365 (3 retailers × 5 products × ~291 weeks) a unique retailer-product-week panel would give.
# - **Why it matters:** β₂ (post-SOE) is estimated from a sample that mechanically removes the cross-status comparisons; the "no widening as enforcement expired" conclusion may partially reflect sample construction rather than behavior. Direction of any bias is not determinable without data.
# - **Recommended fix:** Drop `period` (and `month_fe`) from the join keys; join on `retailer, product, week_seq (, week_start)` only; assign period/regressors at the *pair* or *cell* level afterwards (this requires an explicit decision on how to code a mixed-status pair — e.g., pair-level indicators for "both during", "both post", "mixed" — which is itself the interesting margin).
# - Confirmable from code alone: **yes (mechanism), no (magnitude)**. Author confirmation: **yes**. Server rerun: **yes** (to re-estimate). Fix changes results: **yes**.
#
# ------------------------------------------------------------------------------
# WHAT CHANGED (audit item C1) AND WHY
#
# OLD BEHAVIOR:
#   The pairwise self-join used join keys
#     c("retailer", "product", "week_seq", "week_start", "period", "month_fe").
#   `period` (Pre/During/Post-SOE) is STATE-specific. During the staggered
#   expiration window (Mar 2021 - Nov 2021), a chain could have stores in a
#   post-enforcement state (e.g., LA) and an active-enforcement state (e.g., MS)
#   in the same calendar week. Because `period` was a join key, those
#   cross-status store pairs were silently EXCLUDED from Diff_bar - yet they are
#   exactly the within-chain, cross-state contrast the paper says identifies
#   the post-SOE coefficient. The old collapse also produced two rows per
#   retailer-product-week in straddle weeks, inflating N.
#
# NEW BEHAVIOR (C1 fix):
...
#   Regressions gain a `mixed` dummy (omitted category: Pre-SOE pairs).
#
# ------------------------------------------------------------------------------
